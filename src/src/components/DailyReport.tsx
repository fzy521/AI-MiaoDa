...
  BAUXITE_EAST_SUPPLIERS,
  COAL_SUPPLIERS,
  DEFAULT_SILOS,
  type Role,
  type DailyReport,
  type WeighbridgeItem,
  type TransporterMessage,
  type YardDispatcherReport,
  formatWeighbridgeMessage,
  formatTransporterMessage,
  formatYardDispatcherMessage,
  getBauxiteTotal,
  getCoalTotal,
} from '@/types/report';
import {
  FileText, Clock, CheckCircle2, XCircle, Eye, Send,
  Plus, Trash2, ChevronDown, ChevronUp, BookOpen, MessageSquare,
} from 'lucide-react';

// ─── Shared UI helpers ───

const INPUT_CLS =
  'w-full px-3 py-2 bg-muted/30 border border-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all';
...
            active === t.id ? 'bg-primary text-white shadow-md' : 'bg-muted/50 text-muted-foreground hover:bg-muted'
          )}
        >
          {t.label}
        </button>
...
const NUM_INPUT = 'w-full px-3 py-2 bg-muted/30 border border-border rounded-xl text-sm text-right focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all';

// ─── Role Selector ───

function RoleSelector() {
  const { currentRole, setCurrentRole } = useAuthStore();
  const roles: Role[] = ['WEIGHBRIDGE', 'YARD_DISPATCHER', 'TRANSPORTER', 'DISPATCHER', 'STAFF'];
  return (
    <div className="flex gap-2 flex-wrap">
      {roles.map((r) => (
        <button
...
function WeighbridgeForm({ onPreview }: { onPreview: (msg: string) => void }) {
  const [items, setItems] = React.useState<WeighbridgeItem[]>(
    WEIGHBRIDGE_CUSTOMERS.map((c) => ({ customer: c, vehicles: 0, trips: 0, tons: 0, returnTons: 0 }))
  );
  const [date, setDate] = React.useState(new Date().toISOString().slice(0, 10));
  const { submitReport } = useReportStore();

  const updateItem = (idx: number, field: keyof WeighbridgeItem, value: number | string) =>
    setItems((prev) => prev.map((it, i) => (i === idx ? { ...it, [field]: value } : it)));

  const totalTons = items.reduce((s, it) => s + it.tons, 0);
