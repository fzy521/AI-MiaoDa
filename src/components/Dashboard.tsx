...
import { KPI, RiskLevel } from '../types';
...
const riskDistribution = [
  { name: '重大风险', value: 2, color: 'hsl(0 84% 60%)' },
  { name: '较大风险', value: 8, color: 'hsl(25 90% 60%)' },
  { name: '一般风险', value: 24, color: 'hsl(45 90% 60%)' },
  { name: '低风险', value: 56, color: 'hsl(210 90% 60%)' },
];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {kpiData.map((kpi, i) => (
          <div key={i} className={cn(
            "safety-card p-5",
            kpi.level === 'RED' && "safety-bar-red bg-risk-red-bg/30",
            kpi.level === 'ORANGE' && "safety-bar-orange bg-risk-orange-bg/30",
            kpi.level === 'YELLOW' && "safety-bar-yellow bg-risk-yellow-bg/30",
            kpi.level === 'SUCCESS' && "safety-bar-green bg-success-bg/30"
          )}>
            <div className="flex justify-between items-start mb-2">
              <span className="text-sm font-medium text-muted-foreground">{kpi.label}</span>
              <div className={cn(
                "flex items-center text-xs font-medium",
                kpi.trend > 0 ? "text-risk-red-text" : "text-success-text"
              )}>
                {kpi.trend > 0 ? <TrendingUp size={12} className="mr-1" /> : <TrendingDown size={12} className="mr-1" />}
                {Math.abs(kpi.trend)}%
              </div>
            </div>
            <div className="flex items-baseline gap-1">
              <span className="text-3xl font-bold font-mono tracking-tighter">{kpi.value}</span>
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                />
                <Bar dataKey="hazards" fill="hsl(0 84% 60%)" radius={[4, 4, 0, 0]} />
                <Bar dataKey="fixed" fill="hsl(142 70% 40%)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-muted/50 text-muted-foreground">
              <tr>
                <th className="px-6 py-3 font-medium">隐患编号</th>
                <th className="px-6 py-3 font-medium">隐患描述</th>
                <th className="px-6 py-3 font-medium">风险等级</th>
                <th className="px-6 py-3 font-medium">责任单位</th>
                ...
                <tr key={i} className="hover:bg-accent/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={cn(
                      ...
                      row.level === 'RED' && "bg-risk-red-bg text-risk-red-text border-risk-red-text/20",
                      row.level === 'ORANGE' && "bg-risk-orange-bg text-risk-orange-text border-risk-orange-text/20",
                      row.level === 'YELLOW' && "bg-risk-yellow-bg text-risk-yellow-text border-risk-yellow-text/20"
                    )}>
                  ...
                  <td className="px-6 py-4">
                    <span className={cn(
                      ...
                      row.level === 'RED' ? "text-risk-red-text font-bold" : "text-muted-foreground"
                    )}>
                      <Clock size={14} />
                      {row.time}
                    </span>
